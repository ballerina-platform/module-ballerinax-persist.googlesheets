...
function initSpreadsheet() returns error? {
    ...
    check resetExistingSheets(spreadsheetClient, spreadSheet);
    check addSheetHeaders(spreadsheetClient, spreadSheet);
    rainierClient = check new ();
    rainierClientAllDataType =  check new ();
}

function resetExistingSheets(sheets:Client spreadsheetClient, sheets:Spreadsheet spreadSheet) returns error? {
...
function addSheetHeaders(sheets:Client spreadsheetClient, sheets:Spreadsheet spreadSheet) returns error? {
...
OrderItemExtended orderItemExtended2Updated = {
    orderId: "order-2",
    itemId: "item-2",
    CustomerId: 1,
    paid: true,
